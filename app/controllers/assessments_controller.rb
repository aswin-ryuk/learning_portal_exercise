    ...
    if params.dig(:assessment, :type).present? && params.dig(:assessment, :type_id).present?
      ...
      if type.present?
        ...
        if all_questions.count > 0
          ...
          render json: calculate_remaining_qusts(all_questions, assessment_questions).select(:id, :content, :options)
        else
      ...
      if type.present?
        ...
        if total_qusts_count > 0
          ...
          result = {
            ...
            unseen_questions:((assessment_questions.where(is_skipped: 'Y').count*100)/total_qusts_count)
          }
          render json: result
        else
          render json: {'type': 'No data found for selected type'}
